...
erTopLevel top = case top of
  ...
  TLDef{} -> top
  TLSup w z
    (vs0 :: Vector m0 Id.TVar)
    (t0 :: Type (TFinScope m0 Void))
    (bs0 :: Vector n0 (Bind st (TFinScope m0 Void)))
    ...
    case e0 of
      ...
      ETyAbs _
        ...
        , TUni (vs2 :: Vector m2 Id.TVar) (t2 :: Type (TFinScope m2 Void)) <-
            fmap (strengthenWith absurd0) t0
        , Just Refl <- sameNat (Proxy @m1) (Proxy @m2) ->
            let bs2 :: Vector n0 (Bind st (TFinScope m2 Void))
