module Pukeko.Monomorphic.Checker
  ( checkExpr
  )
  where
...
import Pukeko.Language.Type (Type, (~>), record)
import Pukeko.Monomorphic.Subtyping (isSubtypeOf, join)

import qualified Pukeko.Language.Type        as Type
import qualified Pukeko.Monomorphic.Builtins as Builtins
