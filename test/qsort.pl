...
qsort$ll1 : Int -> Int -> Bool =
  fun (x : Int) (y : Int) -> lt @Int dict$Ord$Int y x
  ...
  fun (xs : List Int) ->
    ...
    | Cons @Int x xs ->
      ...
      | Pair @(List Int) @(List Int) ys zs ->
        append @(List Int) (dict$Monoid$List @Int) (qsort ys) (Cons @Int x (qsort zs))
main$ll1 : List Int -> IO Unit =
  fun (xs : List Int) ->
    traverse_ @Int @IO @List dict$Monad$IO dict$Foldable$List print (qsort xs)
main$ll2 : Int -> IO Unit =
  fun (n : Int) ->
    bind @IO dict$Monad$IO @(List Int) @Unit (sequence @Int @IO dict$Monad$IO (replicate @(IO Int) n input)) main$ll1
