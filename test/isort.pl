...
insert.L1 : Int -> List Int -> List Int =
  fun (y : Int) (xs : List Int) ->
    ...
    | Cons x xs' ->
      match le.L1 @Int ordInt y x with
      | False -> Cons @Int x (insert.L1 y xs')
      | True -> Cons @Int y xs
isort.L1 : List Int -> List Int =
  fun (xs : List Int) ->
    match xs with
    | Nil -> Nil @Int
    | Cons x xs -> insert.L1 x (isort.L1 xs)
main.L1 : List Int -> IO Unit =
  fun (xs : List Int) ->
    traverse_.L2 @Int @IO @List monadIO foldableList print.L1 (isort.L1 xs)
main.L2 : Int -> IO Unit =
  fun (n : Int) ->
